...
class PaymentProvider(BaseProvider):
    ...
    def payment_method(self, customer_segment: str) -> str:
        ...
        return random.choices(
            population=[m[0] for m in methods], weights=[m[1] for m in methods], k=1
        )[0]
    ...
    def payment_status(self, payment_method: str, order_status: str) -> str:
        ...
        if order_status == "completed":
            ...
            return random.choices(
                population=["pending", "failed"], weights=[0.85, 0.15], k=1
            )[0]

    def fraud_score(
        self, payment_method: str, customer_segment: str, order_amount: float
    ) -> float:
    ...
    def currency_code(self) -> str:
        ...
        return random.choices(
            population=[c[0] for c in currencies],
...
class PaymentGenerator:
    ...
    def __init__(self, config: Dict):
        ...
        # Determine starting payment ID
        if config.get("auto_start_id", True):
            self.start_payment_id = self._get_last_payment_id()
            logger.info(f"Auto-detected starting payment ID: {self.start_payment_id}")
        else:
            self.start_payment_id = config.get("start_id", 1)
            logger.info(
                f"Using configured starting payment ID: {self.start_payment_id}"
            )
    ...
    def _get_last_payment_id(self) -> int:
        ...
        try:
            ...
            if not payment_files:
                return 1  # No existing payments, start from 1

            max_payment_id = 0
            for file_key in payment_files:
                # Download and analyze each file
                payment_data = self.storage_client.download_object_as_json(
                    self.config["bucket_name"], file_key
                )

                # Find max payment_id in this file
                file_max = max(record.get("payment_id", 0) for record in payment_data)
                max_payment_id = max(max_payment_id, file_max)

            return max_payment_id + 1

        except Exception as e:
            logger.warning(f"Failed to detect last payment ID, starting from 1: {e}")
            return 1

    def generate_batch(
        self, batch_size: int, batch_num: int, order_data: List[Dict]
    ) -> List[Dict]:
        ...
        for i in range(batch_size):
            payment_id = (
                self.start_payment_id + (batch_num * self.config["batch_size"]) + i
            )
            ...
            order_payment_history[order_id] = payment_history + [
    ...
    def _select_order_for_payment(
        self, order_data: List[Dict], order_payment_history: Dict
    ) -> Optional[int]:
        ...
        for order in order_data:
            ...
            attempt_probability = self.payment_attempt_probabilities.get(
                order_status, 0.5
            )
    ...
    def _generate_payment_for_order(
        self,
        order: Dict,
    ...
    def _generate_first_payment_attempt(
        self, order: Dict, payment_id: int, payment_index: int, batch_num: int
    ) -> Dict:
        ...
        if random.random() < success_prob:
            ...
            payment_status = random.choices(
                population=["failed", "pending"], weights=[0.8, 0.2], k=1
            )[0]
        ...
        fraud_score = self.faker.fraud_score(
            payment_method, customer_segment, order_amount
        )
        ...
        if (
            payment_status == "success"
            and random.random() < self.chargeback_rates[payment_method]
        ):
        ...
        return {
            ...
            "chargeback_date": (
                chargeback_date.strftime("%Y-%m-%d") if chargeback_date else None
            ),
    ...
    def _generate_retry_payment_attempt(
        self,
        order: Dict,
        payment_history: List[str],
        payment_id: int,
        ...
        if random.random() < success_prob:
            ...
            payment_status = random.choices(
                population=["failed", "pending"], weights=[0.9, 0.1], k=1
            )[0]
        ...
        for record in data:
            for field in required_fields:
                if not record.get(field):
                    logger.error(
                        f"Missing required field: {field} in record {record.get('payment_id')}"
                    )
                    return False
            ...
            if record["order_amount"] <= 0:
                logger.error(
                    f"Invalid order amount: {record['order_amount']} in record {record.get('payment_id')}"
                )
                return False
    ...
    def _validate_payment_scenarios(self, payment_data: List[Dict]) -> bool:
        ...
        for order_id, payment_statuses in order_payments.items():
            ...
            if success_count > 1:
                logger.error(
                    f"Order {order_id} has {success_count} successful payments (should be max 1)"
                )
                return False
            ...
            if failed_count > 3:
                ...
                logger.warning(
                    f"Order {order_id} has {failed_count} failed payments (unrealistic retry pattern)"
                )

        logger.info(
            f"Payment scenario validation passed: {len(order_payments)} orders processed"
        )
        return True
    ...
    def _load_reference_data_from_storage(self) -> Tuple[List[Dict], List[Dict]]:
        ...
        for orders_data_object in orders_data_list_objects:
            orders_data_list = self.storage_client.download_object_as_json(
                self.config.get("bucket_name"), orders_data_object
            )
    ...
    def generate_single_payment(self, payment_id: int = None) -> Dict:
        ...
        try:
            ...
            batch_data = self.generate_batch(
                batch_size=1, batch_num=0, order_data=order_data
            )
    ...
    def generate_payments(self, order_data: List[Dict]) -> Dict[str, int]:
        ...
        logger.info(
            f"Starting payment generation: {total_records:,} records in {total_batches} batches"
        )
        ...
        for batch_num in range(total_batches):
            ...
            try:
                batch_data = self.generate_batch(
                    current_batch_size, batch_num, order_data
                )
        ...
        logger.info(
            f"Payment generation completed: {total_records_generated:,} records in {len(files_created)} files"
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--start-id", type=int, default=1, help="Starting payment ID (default: 1)"
    )
    parser.add_argument(
        "--auto-start-id",
        action="store_true",
        default=True,
        help="Automatically detect last payment ID from storage (default: True)",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="payments",
        help="Filepath prefix (default: payments)",
    )
...
def main():
    ...
    config = {
        ...
        "auto_start_id": args.auto_start_id,
