...
class ProductGenerator:
    ...
    def generate_batch(self, batch_size: int, batch_num: int) -> List[Dict]:
        ...
        for i in range(batch_size):
            ...
            supplier_info = random.choices(
                population=self.suppliers, weights=[s[2] for s in self.suppliers], k=1
            )[0]
            ...
            product_data = {
                ...
                "color": random.choice(
                    ["black", "white", "blue", "red", "green", "silver", "gold", "gray"]
                ),
    ...
    def _generate_product_name(
        self, category: str, subcategory: str, brand: str
    ) -> str:
        ...
        return random.choice(
            name_patterns.get(category, [f"{brand} Product {random.randint(100, 999)}"])
        )
        ...
        for record in data:
            for field in required_fields:
                if not record.get(field):
                    logger.error(
                        f"Missing required field: {field} in record {record.get('product_id')}"
                    )
                    return False
            ...
            if record["price"] <= 0:
                logger.error(
                    f"Invalid price: {record['price']} in record {record.get('product_id')}"
                )
                return False

            if record["inventory_level"] < 0:
                logger.error(
                    f"Invalid inventory: {record['inventory_level']} in record {record.get('product_id')}"
                )
                return False
    ...
    def generate_products(self) -> Dict[str, int]:
        ...
        logger.info(
            f"Starting product generation: {total_records:,} records in {total_batches} batches"
        )
        ...
        logger.info(
            f"Product generation completed: {total_records_generated:,} records in {len(files_created)} files"
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--start-id", type=int, default=1, help="Starting product ID (default: 1)"
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="products",
        help="Filepath prefix (default: products)",
    )
