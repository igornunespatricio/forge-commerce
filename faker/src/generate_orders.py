...
class OrderProvider(BaseProvider):
    ...
    def order_status(self) -> str:
        ...
        return random.choices(
            population=[s[0] for s in statuses], weights=[s[1] for s in statuses], k=1
        )[0]
    ...
    def payment_status(self, order_status: str) -> str:
        ...
        if order_status == "completed":
            ...
            return random.choices(
                population=["pending", "failed"], weights=[0.80, 0.20], k=1
            )[0]
    ...
    def discount_percentage(self, customer_segment: str) -> float:
        ...
        if customer_segment == "premium":
            ...
            return round(
                random.choices([0, 5, 10, 15], weights=[40, 30, 20, 10])[0] / 100, 2
            )
    ...
    def order_date(
        self, start_date: datetime, end_date: datetime, customer_segment: str
    ) -> datetime:
...
class OrderGenerator:
    ...
    def __init__(self, config: Dict):
        ...
        # Determine starting order ID
        if config.get("auto_start_id", True):
            self.start_order_id = self._get_last_order_id()
            logger.info(f"Auto-detected starting order ID: {self.start_order_id}")
        else:
            self.start_order_id = config.get("start_id", 1)
            logger.info(f"Using configured starting order ID: {self.start_order_id}")
    ...
    def _get_last_order_id(self) -> int:
        ...
        try:
            ...
            if not order_files:
                return 1  # No existing orders, start from 1

            max_order_id = 0
            for file_key in order_files:
                # Download and analyze each file
                order_data = self.storage_client.download_object_as_json(
                    self.config["bucket_name"], file_key
                )

                # Find max order_id in this file
                file_max = max(record.get("order_id", 0) for record in order_data)
                max_order_id = max(max_order_id, file_max)

            return max_order_id + 1

        except Exception as e:
            logger.warning(f"Failed to detect last order ID, starting from 1: {e}")
            return 1
    ...
    def generate_batch(
        ...
        for i in range(batch_size):
            order_id = self.start_order_id + (batch_num * self.config["batch_size"]) + i
            ...
            order_date = self.faker.order_date(
                self.config["start_date"], self.config["end_date"], customer_segment
            )
            ...
            fulfillment_date = self._calculate_fulfillment_date(
                order_date, shipping_method
            )
            delivery_date = self._calculate_delivery_date(
                fulfillment_date, shipping_method
            )
            ...
            order_data = {
                ...
                "fulfillment_date": (
                    fulfillment_date.strftime("%Y-%m-%d") if fulfillment_date else None
                ),
                "delivery_date": (
        ...
        for _ in range(quantity):
            ...
            if not category_products:
                ...
                category_products = [
                    p
                    for p in product_lookup.values()
                    if p["is_active"] and p["inventory_level"] > 0
                ]
    ...
    def _calculate_fulfillment_date(
        self, order_date: datetime, shipping_method: str
    ) -> datetime:
    ...
    def _calculate_delivery_date(
        self, fulfillment_date: datetime, shipping_method: str
    ) -> datetime:
        ...
        for record in data:
            for field in required_fields:
                if not record.get(field):
                    logger.error(
                        f"Missing required field: {field} in record {record.get('order_id')}"
                    )
                    return False
            ...
            if record["total_amount"] <= 0:
                logger.error(
                    f"Invalid total amount: {record['total_amount']} in record {record.get('order_id')}"
                )
                return False
    ...
    def generate_single_order(
        self, order_id: int = None, customer_id: int = None
    ) -> Dict:
    ...
    def _load_reference_data_from_storage(self) -> Tuple[List[Dict], List[Dict]]:
        ...
        for customer_data_object in customer_data_list_objects:
            customer_data_list = self.storage_client.download_object_as_json(
                self.config.get("bucket_name"), customer_data_object
            )
        ...
        for product_data_object in product_data_list_objects:
            product_data_list = self.storage_client.download_object_as_json(
                self.config.get("bucket_name"), product_data_object
            )
    ...
    def generate_orders(
        self, customer_data: List[Dict], product_data: List[Dict]
    ) -> Dict[str, int]:
        ...
        logger.info(
            f"Starting order generation: {total_records:,} records in {total_batches} batches"
        )
        logger.info(
            f"Using {len(customer_data):,} customers and {len(product_data):,} products"
        )
        ...
        for batch_num in range(total_batches):
            ...
            try:
                batch_data = self.generate_batch(
                    current_batch_size, batch_num, customer_data, product_data
                )
        ...
        logger.info(
            f"Order generation completed: {total_records_generated:,} records in {len(files_created)} files"
        )
...
def parse_arguments():
    ...
    parser.add_argument(
        "--start-id", type=int, default=1, help="Starting order ID (default: 1)"
    )
    parser.add_argument(
        "--auto-start-id",
        action="store_true",
        default=True,
        help="Automatically detect last order ID from storage (default: True)",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="orders",
        help="Filepath prefix (default: orders)",
    )
